  ...
  document.querySelectorAll('.location-card').forEach(card => {
    card.addEventListener('click', () => {
      ...
      data.markers.forEach(m => {
        ...
        const residentsInLot = residents.filter(r => r.block === data.block && r.lot === lotNumber);

        let pinColorClass;

        if (residentsInLot.length === 0) {
          pinColorClass = "no-resident";   // yellow
        } else if (residentsInLot.every(r => r.status === "inactive")) {
          pinColorClass = "inactive-resident"; // red
        } else {
          pinColorClass = "active-resident";   // green
        }

        const customIcon = L.divIcon({
          className: `custom-pin ${pinColorClass}`,
          html: `<div class="pin"></div>`,
          iconSize: [20, 20],
          iconAnchor: [10, 20]
        });

        const marker = L.marker(m.pos, { icon: customIcon }).addTo(markersLayer);
        ...
        marker.on('click', () => {
          openLotModal(data.block, lotNumber);
        });
